...
Curriculum structure complete — all **31 phases** with tools, resources, tasks, deliverables, and checklists. All three tracks carry full `## Lesson` sections: the 9 IT phases, all 15 cyber phases (8 core plus 7 depth modules), and the 7 mid-level `advance-roadmap` phases — **494,386 phase-file words** in total. The learning site renders those lessons directly, with a per-lesson table of contents, and loads each one on demand. The site includes the dashboard, tools library, portfolio and application trackers, certifications, practice exams, in-lesson search, Shared, Your work, Where you've been, and a time-aware "what should I do today?" view. CI runs content-integrity checks, **14 site test suites**, and a separate Chromium browser suite on every push and pull request; Gecko and WebKit remain a known browser-coverage gap. The exact checks are listed in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
