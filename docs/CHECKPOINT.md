...
|---|---|
| Branch | `main` |
| Tracked files | **145** — the 140 of the previous pass plus this pass's five: `scripts/audit-terms.mjs` (the acronym measurement, which always exits 0), `scripts/audit-refs.mjs` (the cross-reference guard, which gates), `docs/COMPREHENSION-AUDIT.md` (the audit's findings), `learning-site/scripts/browser-check.mjs` (the real-engine render check), and `scripts/audit-time-budget.mjs` (the time-budget guard, which gates — added because the second comprehension pass found the arithmetic class again, in eight phases). `lint-content` reports the same 145 |
| Working tree | **Uncommitted, and this pass is the reason.** `git status --short` shows the time-budget guard and its wiring — `scripts/audit-time-budget.mjs` (untracked, new), `.github/workflows/ci.yml`, `docs/DECISIONS.md`, `docs/WORKFLOW.md`, `docs/ROADMAP.md`, `docs/SESSION-LOG.md`, `CHANGELOG.md` and this file — alongside the second comprehension pass's phase-file edits, which were also still uncommitted when this entry was written. Nothing here has been committed or pushed yet, so **the numbers below are from a working tree, not from a commit**, which is the distinction this file exists to keep honest |
| Unpushed | **Yes — everything above.** `main` and `origin/main` were level at `b93e3f7` before this pass began; run `git --no-pager log --oneline -n 1` for the current hash and `git --no-pager log origin/main..HEAD --oneline` for what is ahead, since this file cannot contain its own |
...
| Checks | `lint-content` **145** files / 0 issues; `audit-content` 0 issues across 23 phases; `audit-lesson-ast` 23 lessons / 0 loss; `audit-readability` 0 of 23 outside target, **0 paragraphs over the 110-word ceiling and 0 over the 90-word editorial target** (gate lowered 150 → 110 in D-024, longest paragraph 90 IT / 86 cyber); **`audit-refs` 0 broken cross-references** (new, gating); **`audit-time-budget` 23 phases / 0 findings** (new, gating — a stated total that disagrees with its own breakdown table, a phase calling itself the heaviest when a later one budgets three times as much, a track overview listing 4 weeks where every phase file says 6); `test:highlight` 37; `test:ui` 42; `test:data` 86; `test:notes` 28; `test:work` 31; `test:today` 77; `test:lesson-search` 77; `test:search` 41; `test:smoke` 181 renders / 0 failures; **`test:browser` 50 checks / 0 failures** (new, real engine); production build clean. Re-run in full after the final commit — every number above is from that run |
...
| Paragraph density | **Zero paragraphs over 90 words anywhere, in either track — and the gate now sits at 110, not 150** (D-024). The 33 walls of text that accumulated in the IT track were split at natural seams with every word preserved — proved by `git diff --word-diff` — and the four paragraphs still standing over the editorial target (IT 01 91, IT 03 95, IT 05 94, IT 07 93) were split the same way in this pass. With the backlog at zero the gate moved down from 150 to **110**; a gate 60 words above the standard it protects could not fail on prose a reader would actually find hard to read. `EDITORIAL = 90` and `CEILING = 110` are named constants, so the report and the gate cannot drift apart. Longest paragraph is **90 (IT) / 86 (cyber)** |
...
- [ ] **Nobody has timed a single curriculum task, and every time budget in the repository is an author's estimate.** This is the largest untested claim in the project and the second pass made it sharper rather than softer: a stated 60–80 hours was found sitting above its own parts summing to 40–56 (cyber 09), and IT 04 stated two different phase totals in one file. The arithmetic is now consistent everywhere, which is *not* the same as the estimates being right — every figure is still a guess no reader has measured. Closing it needs one reader with a timer and a single phase, not more editing.
...
- **Zero paragraphs over 90 words, in either track — and the hard gate is now 110, not 150** (D-024). The four paragraphs that stood over the editorial target (IT 01 91, IT 03 95, IT 05 94, IT 07 93) were split, every word preserved. Longest paragraph is **90 (IT) / 86 (cyber)**.
- **The gate sits at 110 rather than at the 90-word editorial target, and that gap is deliberate.** The four paragraphs split in this pass exist because the text got *easier* to follow, not looser — a gate at 90 would fail on every clarification a writer adds. 110 fails on prose a reader has to re-read and passes prose that merely wants splitting: the failure that keeps a gate honest is one it can pass.
...
Expected results: lint `145 files, 0 issues`; content audit `0 issues`; lesson AST `23 lessons, 0 loss`; readability `0 of 23` outside target with `0 paragraphs over 110 words` and `0 over the 90-word editorial target`; cross-references `0 broken`; time budgets `23 phases checked, 0 findings`; smoke `181 renders across 23 phases and 191 tools, 0 failures`; highlight `37 checks, pass`; ui `42 checks, pass`; data `86 checks, pass`; notes `28 checks, pass`; work `31 checks, pass`; today `77 checks, pass`; lesson-search `77 checks, pass`; search `41 checks, pass`; browser `50 checks, pass` (needs a preview server running — CI starts and stops one inside the step).