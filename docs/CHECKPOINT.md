...
| Working tree | **Clean once this file is committed.** The CI change landed in `34f7ee3`: `.github/workflows/ci.yml` (six suite steps added to the site job — see the "Guards in CI" row), `CHANGELOG.md` and `docs/SESSION-LOG.md` writing it up in the same words, and this file's corrections. The feature work landed in `3d637e7`, with the `shared.json` contract correction in `CONTENT-SCHEMA.md` in `94fa18c`. The only uncommitted change at any moment is this file's own edit, because it cannot contain its own hash |
| Unpushed | None — `main` and `origin/main` are level. Run `git --no-pager log --oneline -n 1` for the exact hash, since this file cannot contain its own. The pass that wired the full suite into CI landed in `34f7ee3` |
...
| CI | `.github/workflows/ci.yml` — two jobs on push to `main` and on PRs (D-008). Content job: lint, content audit, lesson-parser audit, readability audit. Site job: install, build, `test:smoke`, `test:search`, `test:highlight`, `test:ui`, `test:data`, `test:notes`, `test:work`, `test:today`, `test:lesson-search`. **`cyber-restructure-check.mjs` is deliberately excluded**: it reads a before-snapshot from `process.env.TEMP`, which is not in the repository, so it can only ever fail on a fresh clone. It is a one-off migration tool for a restructure pass, not a standing guard. CI run **#38 on `34f7ee3` is green** and its job list shows **every one of those steps executed and passed** — the workflow file CI ran is the one on disk |
...
1. **The CI change is committed, pushed and observed green — nothing is outstanding here.** `.github/workflows/ci.yml` and its documentation landed in `34f7ee3`; CI run **#38** on that hash is `completed / success` with all six new steps listed as passing, and the Pages deploy succeeded on the same commit. This item existed because "CI is green" and "these six steps have run in CI" were once two different claims; they are now one. See the CI row and the "Guards in CI" row above.
