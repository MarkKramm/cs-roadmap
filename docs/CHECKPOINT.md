...
| Working tree | **Clean once this file is committed.** This pass landed in five commits: `d49c975` (the comprehension-audit content fixes plus the audit document), `aeb9fc3` (the cross-reference guard, wired into CI, plus the cyber 10–12 fixes), `58b941c` (a checklist row that claimed a paid feature was free), `fd1d841` (IT 03–04), and `e4f2060` (the browser check, D-023, and its CI step). The only uncommitted change at any moment is this file's own edit, because it cannot contain its own hash |
...
| Checks | `lint-content` **143** files / 0 issues; `audit-content` 0 issues across 23 phases; `audit-lesson-ast` 23 lessons / 0 loss; `audit-readability` 0 of 23 outside target and 0 paragraphs over 150 words; **`audit-refs` 0 broken cross-references** (new); `test:highlight` 37; `test:ui` 42; `test:data` 86; `test:notes` 28; `test:work` 31; `test:today` 77; `test:lesson-search` 77; `test:search` 41; `test:smoke` 181 renders / 0 failures; **`test:browser` 50 checks / 0 failures** (new); production build clean |
...
npm run test:browser                     # render the six views in a real engine
```
