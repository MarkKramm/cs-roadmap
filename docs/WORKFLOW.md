...
|---|---|---|
| Content integrity | `lint-content` → `audit-content` → `audit-lesson-ast` → `audit-readability` → `audit-refs` → `audit-time-budget` → `audit-changelog` → `test-audit-changelog` → `audit-terms --self-test` | repository root |
