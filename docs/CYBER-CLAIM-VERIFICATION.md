...
| Security tool commands and flags | 99 | 93 | **0** | 6 |
...
them, and lowest in the mechanical classes — ATT&CK identifiers, CVE and cryptography — where a
line either names a real technique or does not. The pack's own rules say not to invent a verdict
to fill a column.

**Both command-shaped classes were predicted to be the highest-yield and both returned no defects.**
That is worth recording as a result rather than a non-event. On the IT pass, 3 of 3 real defects
were "right name, broken invocation" — a real command carrying a flag that does not exist — and
these two classes (159 rows together) are exactly that shape. They came back clean. **A prediction
that a class will fail is a claim about the shape of the risk, not about the content, and 159 clean
rows is evidence that this content was written carefully** — not evidence the prediction was wrong.
Recording it matters, because a high-yield prediction quietly dropped when it fails teaches nothing
and would make the next prediction worthless.
...
**One claim that looked like a defect turned out to be a taught trap, on purpose.**
`02-phase-networking-and-linux.md:1325` says `journalctl -u sshd` matches nothing on distributions
where the unit is `ssh.service`. This reads like a broken instruction, because the same phase tells
the reader to run `journalctl -u sshd` at lines 747 and 755. It is deliberate: line 1318 poses it as
a troubleshooting exercise — *"`journalctl -u sshd` returns nothing, but `ssh` clearly works and logs
exist. What is the most likely explanation?"* — and 1325 is the answer. **The lesson is on the page,
not in the command.** Checking the surrounding text before calling a wrong-looking command a defect
is what separated this from a finding.
...
> derives every published figure from the rows and CI fails if the table disagrees — **and fails if
> any row anywhere in the document still has an empty verdict column, whether or not its class is
> listed above.**

**Every ATT&CK ID in the corpus was re-checked against attack.mitre.org**, because a misattributed
technique number is exactly the kind of claim that reads authoritative and is silently wrong. All 34
IDs and their tactics are correct. **Two rows name the parent technique rather than the
sub-technique** in the Sub-technique column — `T1098.001` reads *"Account Manipulation"* where MITRE
says *"Additional Cloud Credentials"*, and `T1087.001` reads *"Account Discovery"* where MITRE says
*"Local Account"*. Both are left as written, deliberately: the ID, the tactic and the family are all
correct, `T1098` in the preceding column plus `.001` resolves to the right page, and for a beginner
the family name is the more legible label. **This is a recorded judgement, not an oversight** — the
alternative was changing two rows to match nine others and losing the plainer English.
...
**"DNS record types" sat unverified through five completed passes while every guard reported clean.**
The class was added to `split-claims.mjs` already marked `done: true` with `doneThrough: 4` — a
figure carried over from the **IT** track, whose version of that class genuinely has 4 rows. The
cyber version has **3 different rows**, so they were never sent to a verifier; and because the class
was marked done it was also **omitted from the `DONE` list that `audit-verdict-counts.mjs` totalled**.
The published total was therefore self-consistent and wrong: it summed the classes it had been told
about, and the missing class was invisible by construction. It surfaced only when a final sweep
counted rows with **empty** verdict columns instead of rows already filled.

**This is D-038 again, in the one place I had claimed to have fixed it** — a worklist that cannot
distinguish done from outstanding, and a record whose own summary is a claim. The lesson is not
"check twice": it is that **counting the classes you remembered cannot detect the class you forgot.**
The guard now scans every `## ` section in the document, listed or not, and fails on any empty
verdict column.

A second, smaller bug sat behind it: the splitter could drop a contiguous prefix of verified rows
(`doneThrough`) but could not express *"rows 1–3 outstanding, row 4 done"*, which is exactly this
class's shape. Re-emitting it would have re-sent row 4 — settled work — to a verifier. Rows that
already carry a verdict are now dropped individually, independent of any prefix cut.
...
| # | Location | Text as written | Verdict |
|---|---|---|---|
| 1 | `02-phase-networking-and-linux.md:26` | - Understand IPv4, IPv6 basics, subnetting, DNS, DHCP, NAT, TCP/UDP, ICMP, TLS, HTTP, and common ports. | **UNVERIFIABLE** — curriculum learning objective |
| | | <sub>↓ - Use Linux confidently for files, permissions, processes, services, SSH, logs, and networking.</sub> | |
| 2 | `02-phase-networking-and-linux.md:41` | - TCP handshake and connection teardown | **OK** — RFC 9293: “this is called the three-way (or three message) handshake (3WHS)”. |
| | | <sub>↑ - DHCP process<br>↓ - UDP use cases</sub> | |
| 3 | `02-phase-networking-and-linux.md:132` | \| 4 Transport \| Ports, reliability \| TCP, UDP \| Port closed, timeout \| | **OK** — OSI model: Layer 4 (Transport) handles ports and reliability with TCP and UDP. |
...
| 4 | `02-phase-networking-and-linux.md:342` | **DHCP** (Dynamic Host Configuration Protocol) hands out addresses automatically, through a four-step exchange known as **DORA**. | **OK** — DHCP: DORA is DISCOVER, OFFER, REQUEST, ACK. |
| 5 | `02-phase-networking-and-linux.md:366` | **TCP** is connection-oriented and reliable. It begins with the **three-way handshake**: | **OK** — RFC 9293: the three-way handshake establishes a TCP connection. |
| 6 | `02-phase-networking-and-linux.md:369` ▶ | Client → Server: SYN ("I would like to connect, my sequence starts here") | **OK** — RFC 9293: “A --> B SYN my sequence number is X”. |
| | | <sub>↑ ```text<br>↓ Server → Client: SYN-ACK ("Acknowledged, and here is mine")</sub> | |
| 7 | `02-phase-networking-and-linux.md:370` ▶ | Server → Client: SYN-ACK ("Acknowledged, and here is mine") | **OK** — RFC 9293: steps 2 and 3 can be combined in a single message — the SYN-ACK. |
...
| 8 | `02-phase-networking-and-linux.md:383` | \| A `SYN` that gets no reply \| The port is filtered, or the host is down \| | **OK** — Nmap: no response to a SYN means the port is filtered or the host is down. |
...
| 9 | `02-phase-networking-and-linux.md:384` | \| A `SYN` answered by `RST` (reset) \| The host is alive but nothing is listening on that port \| | **OK** — Nmap: a RST response means the host is up but nothing is listening on that port. |
| | | <sub>↑ \| A `SYN` that gets no reply \| The port is filtered, or the host is down \|</sub> | |
| 10 | `02-phase-networking-and-linux.md:392` | **UDP** is connectionless and unreliable: no handshake, no ordering, no retransmission. | **OK** — RFC 768: UDP is connectionless, with no handshake, ordering or retransmission. |
| 11 | `02-phase-networking-and-linux.md:424` | Two of these deserve emphasis. **Port 445 (SMB)** and **port 3389 (RDP)** exposed to the internet are among the most common causes of ransomware incidents in the world, because both are legitimate services that attackers can authenticate to. | **OK** — IANA: port 445 is SMB and port 3389 is RDP. |
| 12 | `02-phase-networking-and-linux.md:861` ▶ | sudo ss -tulpn # listening TCP/UDP ports with owning processes | **OK** — iproute2: `ss -tulpn` lists listening TCP/UDP sockets with owning processes. |
| | | <sub>↑ ip neigh # the ARP table — IP-to-MAC mappings<br>↓ dig example.com MX # DNS records</sub> | |
| 13 | `02-phase-networking-and-linux.md:955` ▶ | sudo tcpdump -i eth0 -n 'tcp port 80' # only HTTP, no name resolution | **OK** — tcpdump: `-i eth0 -n 'tcp port 80'` captures HTTP without name resolution. |
| | | <sub>↑ sudo tcpdump -i eth0 port 53 # only DNS<br>↓ sudo tcpdump -r capture.pcap -nn # read a saved capture</sub> | |
| 14 | `02-phase-networking-and-linux.md:970` ▶ | tcp.flags.syn == 1 connection attempts | **OK** — Wireshark: `tcp.flags.syn == 1` matches SYN packets. |
| | | <sub>↑ ip.addr == 192.168.1.50 one host<br>↓ tcp.flags.reset == 1 resets — refused connections</sub> | |
| 15 | `02-phase-networking-and-linux.md:1014` | \| **open** \| A completed TCP handshake, or a UDP reply \| Something is listening and accepted the connection \| | **OK** — Nmap: an open port completes the TCP handshake, or replies over UDP. |
| 16 | `02-phase-networking-and-linux.md:1136` | - **TCP's three-way handshake explains Nmap's output.** Completed handshake = open; `RST` = closed (host is up); silence = filtered. Learn the handshake and port states stop being arbitrary. | **OK** — RFC 9293: completed handshake = open, RST = closed, silence = filtered. |
| 17 | `02-phase-networking-and-linux.md:1159` | 9. **Write the TCP handshake mini-report last** (task 9), once the capture work has given you the material. Follow the deliverable's structure and keep it short. If you can explain the handshake from your own capture — including why the packets are in that ord … | **UNVERIFIABLE** — curriculum task instruction |
| 18 | `02-phase-networking-and-linux.md:1175` | \| Wireshark \| Packet analysis \| Free \| https://www.wireshark.org/ \| Capture DNS, HTTP, TCP handshake \| tcpdump \| | **OK** — Wireshark is a free packet analyser; the cited URL is its home page. |
| 19 | `02-phase-networking-and-linux.md:1200` | 9. Write a mini-report explaining one TCP handshake capture. <!-- id: cyber-02-t09 band: focused energy: normal --> | **UNVERIFIABLE** — curriculum task instruction |
| 20 | `02-phase-networking-and-linux.md:1307` | **Why:** The authoritative server is the source of truth for its zone and answers fresh every time, while a recursive resolver returns whatever it has cached until the TTL expires. So a disagreement between them is usually a timing difference rather than a con … | **OK** — RFC 1034/1035: an authoritative server is the source of truth for its zone; a recursive resolver returns cached answers until the TTL expires. |
| 21 | `03-phase-security-fundamentals.md:466` | It also explains why the *direction* of a rule is essential context. RDP on port 3389 is an *inbound* service — the thing you are protecting — so an inbound deny on 3389 is the correct posture, and a separate outbound permit on 3389 is ordinary and unrelated t … | **OK** — IANA: port 3389 is RDP, an inbound service. |
| 22 | `03-phase-security-fundamentals.md:500` | If TLS hides the content, signature matching cannot see inside without interception. This is why modern detection leans on metadata — connection patterns, volumes, timing, destinations, and JA3/JA4 fingerprints of the TLS handshake itself — rather than payload … | **OK** — JA3 and JA4 are TLS fingerprinting methods derived from handshake metadata. |
| 23 | `03-phase-security-fundamentals.md:549` | \| DNS-over-HTTPS clients \| The lookup is encrypted and leaves the network's resolver entirely \| | **OK** — RFC 8484: DNS-over-HTTPS carries DNS queries over HTTPS, bypassing the network resolver. |
...
| 24 | `03-phase-security-fundamentals.md:956` | \| 5 \| Check for old app passwords and OAuth grants \| Access you granted years ago and forgot \| | **UNVERIFIABLE** — security hygiene advice |
| | | <sub>↑ \| 4 \| Find your recovery options for each \| A stale recovery email is an unlocked back door \|</sub> | |
| 25 | `06-phase-portfolio-projects.md:45` | - Capture DNS, HTTP, TLS handshake metadata, and failed connection attempts. | **UNVERIFIABLE** — curriculum task outline |
| | | <sub>↓ - Explain protocols and suspicious indicators.</sub> | |
| 26 | `06-phase-portfolio-projects.md:530` | features. The report becomes *"here is a DNS query, here is a TCP handshake, | **UNVERIFIABLE** — curriculum narrative |
...
| 27 | `06-phase-portfolio-projects.md:542` | \| **A name-resolution investigation** \| Browse to a domain: the DNS query and response, the resolved address, the TCP handshake, the TLS negotiation including SNI, the HTTP request if unencrypted \| Which resolver was used? Was the response cached? Did anyth … | **OK** — A DNS query and response, the TCP handshake, and a TLS ClientHello are real, observable network events. |
| 28 | `06-phase-portfolio-projects.md:555` | \| `tcp.flags.syn == 1 && tcp.flags.ack == 0` \| Connection attempts \| | **OK** — Wireshark: `tcp.flags.syn == 1 && tcp.flags.ack == 0` matches initial SYN packets. |
| | | <sub>↑ \| `dns` \| Name-resolution activity \|<br>↓ \| `tls.handshake.extensions_server_name` \| Which hostname was requested over TLS \|</sub> | |
| 29 | `08-phase-job-application.md:300` | **TCP — "Why does understanding the TCP three-way handshake matter for security analysis?"** | **UNVERIFIABLE** — interview preparation framing |
| 30 | `08-phase-job-application.md:302` | *"Because it tells you whether a connection actually completed, which distinguishes a scan or a blocked attempt from a real session. The handshake is SYN, SYN-ACK, ACK.* | **OK** — RFC 9293: the three-way handshake is SYN, SYN-ACK, ACK. |
| 31 | `08-phase-job-application.md:304` | *In a capture, a full handshake means the host connected successfully. A SYN with no response means the port is filtered or the host is down. A SYN followed by RST means the port is closed but the host is reachable — that pattern across many ports is character … | **OK** — Nmap: a completed handshake means connected, no response means filtered or down, and SYN then RST means closed. Standard port-state interpretation. |
| 32 | `12-phase-scripting-automation.md:89` | \| Authentication flows \| API keys, bearer tokens, OAuth — the same patterns, new names \| | **UNVERIFIABLE** — curriculum topic outline |
...
| 33 | `14-phase-web-app-security.md:1282` | - **Injection is about an interpreter, not about SQL.** NoSQL, LDAP, template, command, and header injection are the same bug in different places. | **UNVERIFIABLE** — conceptual claim about the class of injection bugs, not a protocol behaviour |
| 34 | `15-phase-ot-ics-security.md:87` | \| TCP/IP, ports, sessions, and handshakes \| Nearly all modern industrial traffic rides on TCP or UDP \| | **UNVERIFIABLE** — general characterisation of industrial protocol usage |
| | | <sub>↑ \|---\|---\|<br>↓ \| Wireshark and reading a capture \| The core skill of this phase, unchanged in method \|</sub> | |
| 35 | `15-phase-ot-ics-security.md:330` | \| **Modbus TCP** \| Modicon, 1979, over serial first \| TCP, port 502 \| Read and write registers and coils on a device \| **None** \| | **OK** — Modbus TCP runs on port 502 and has no authentication. |
| 36 | `15-phase-ot-ics-security.md:331` | \| **DNP3** \| Utilities, early 1990s \| TCP or UDP, port 20000 \| Master-to-outstation telemetry for electric and water utilities \| Optional, and often unused \| | **OK** — DNP3 runs on port 20000 over TCP or UDP, with optional and often unused authentication. |
| 37 | `15-phase-ot-ics-security.md:332` | \| **EtherNet/IP** \| Rockwell and ODVA, late 1990s \| TCP port 44818, UDP port 2222 \| Industrial device messaging using the CIP object model \| **None** by default \| | **OK** — EtherNet/IP uses TCP 44818 and UDP 2222, with the CIP object model. |
| 38 | `15-phase-ot-ics-security.md:334` | \| **OPC UA** \| The OPC Foundation, 2006 onward \| TCP, commonly port 4840 \| Vendor-neutral data modelling and transport \| **Yes**, with certificates and sessions \| | **OK** — OPC UA commonly uses TCP port 4840 and supports certificates and sessions. |
| 39 | `15-phase-ot-ics-security.md:342` | A Modbus TCP packet begins with a seven-byte header, then a function code, then data. | **OK** — Modbus TCP: the MBAP header is 7 bytes, followed by a function code and data. |
| 40 | `15-phase-ot-ics-security.md:446` | \| **Implicit** \| UDP, port 2222, multicast \| Repeated at the control loop rate, carrying I/O data \| A heartbeat that never stops \| | **OK** — EtherNet/IP implicit messaging uses UDP port 2222. |
| 41 | `15-phase-ot-ics-security.md:447` | \| **Explicit** \| TCP, port 44818 \| Request and response, for configuration and diagnostics \| A conversation \| | **OK** — EtherNet/IP explicit messaging uses TCP port 44818. |
| 42 | `15-phase-ot-ics-security.md:717` | \| A TCP SYN to an unopened port \| Some older stacks crash or hang on unexpected packets \| | **OK** — Legacy OT stacks are documented as crashing or hanging on unexpected packets. |
| | | <sub>↑ \|---\|---\|<br>↓ \| A service probe that speaks half a protocol \| A device may enter a fault state waiting for the rest \|</sub> | |
| 43 | `15-phase-ot-ics-security.md:767` | \| `ip.src == <controller> && tcp.flags.syn == 1` \| New connections to a controller — the highest-value alert \| | **OK** — Wireshark: `ip.src == <addr> && tcp.flags.syn == 1` filters new connections from one source. |
| 44 | `15-phase-ot-ics-security.md:802` | Read the rule as a sentence. It matches established TCP to port 502, checks that the protocol identifier field is zero, then tests the byte at offset seven — the function code — and alerts when it is fifteen or above. | **OK** — Modbus TCP: the protocol identifier field is 0 and the function code sits at offset 7. |
...
| 2 | `01-phase-foundations.md:840` | **Why:** The exploit is the specific technique or piece of code that takes advantage of a vulnerability, and the crafted URL is exactly that. The vulnerability is the flaw in the outdated Apache version itself — it exists whether or not anyone writes a URL. Th … | **UNVERIFIABLE** — conceptual explanation of exploit versus vulnerability |
...
| 5 | `02-phase-networking-and-linux.md:517` | \| **`/etc`** \| `/etc/passwd` (user accounts), `/etc/shadow` (password hashes, readable only by root), `/etc/ssh/sshd_config` (SSH server configuration), `/etc/hosts` \| Configuration is where misconfiguration is found \| | **OK** — FHS: /etc/passwd, /etc/shadow and /etc/ssh/sshd_config are the standard paths; /etc/shadow is root-only. |
| 6 | `02-phase-networking-and-linux.md:518` | \| **`/var/log`** \| The logs you will investigate \| The evidence lives here \| | **OK** — FHS: /var/log holds log files. |
...
| 7 | `02-phase-networking-and-linux.md:625` | Every `sudo` invocation is logged, which is why it also serves the **accounting** part of AAA. Reading `/etc/sudoers` shows who has been granted what — and a common finding in security reviews is that far too many users are in the sudo group. | **OK** — /etc/sudoers defines sudo privileges. |
| 8 | `02-phase-networking-and-linux.md:645` | Reading `/etc/passwd` carefully is a genuinely useful skill. Two fields carry the security meaning. | **OK** — /etc/passwd contains user account information. |
| 9 | `02-phase-networking-and-linux.md:652` | \| **Login shell** \| `/usr/sbin/nologin` \| The account exists but cannot log in interactively — how service accounts are locked down \| | **OK** — /usr/sbin/nologin prevents an interactive login and is the standard service-account shell. |
...
| 11 | `02-phase-networking-and-linux.md:738` | \| `/var/log/auth.log` \| Authentication: logins, `sudo` use, SSH attempts (Debian/Ubuntu) \| | **OK** — Debian/Ubuntu: /var/log/auth.log records authentication, sudo and SSH. |
| | | <sub>↑ \|---\|---\|<br>↓ \| `/var/log/secure` \| The same role on RHEL/CentOS family \|</sub> | |
| 12 | `02-phase-networking-and-linux.md:739` | \| `/var/log/secure` \| The same role on RHEL/CentOS family \| | **OK** — RHEL/CentOS: /var/log/secure is the authentication log. |
...
| 13 | `02-phase-networking-and-linux.md:740` | \| `/var/log/syslog` \| General system messages (Debian/Ubuntu) \| | **OK** — Debian/Ubuntu: /var/log/syslog holds general system messages. |
| | | <sub>↑ \| `/var/log/secure` \| The same role on RHEL/CentOS family \|<br>↓ \| `/var/log/messages` \| General system messages (RHEL/CentOS) \|</sub> | |
| 14 | `02-phase-networking-and-linux.md:741` | \| `/var/log/messages` \| General system messages (RHEL/CentOS) \| | **OK** — RHEL/CentOS: /var/log/messages holds general system messages. |
...
| 15 | `02-phase-networking-and-linux.md:742` | \| `/var/log/apache2/`, `/var/log/nginx/` \| Web server access and error logs \| | **OK** — /var/log/apache2/ and /var/log/nginx/ hold web server access and error logs. |
...
| 17 | `02-phase-networking-and-linux.md:839` | `zgrep` searches compressed logs too. On a system that has been running a while, `/var/log/auth.log` is only the most recent slice, and the earlier evidence is in `auth.log.1.gz`, `auth.log.2.gz` and so on. `auth.log*` catches all of them. | **OK** — logrotate names rotated logs .1.gz, .2.gz and so on, so the glob `auth.log*` catches them. |
| 18 | `02-phase-networking-and-linux.md:1140` | - **`/etc/passwd` UID 0 is root, and nothing else should be.** A non-root account with UID 0 is a backdoor; a service account with `/bin/bash` where `nologin` belongs is a misconfiguration. | **OK** — /etc/passwd: UID 0 is root and nothing else should hold it. |
| 19 | `02-phase-networking-and-linux.md:1152` | 2. **Then permissions** (task 2), because they are the foundation of Linux security thinking. Create two users, put them in a group, create a file, and try to read it as the wrong user. Read `/etc/passwd` and find the UID field. Being denied access is the less … | **OK** — /etc/passwd contains the UID field. |
| 20 | `02-phase-networking-and-linux.md:1153` | 3. **Then SSH** (task 3), which turns your VM into a remote system you can work with comfortably. Set up keys, not just passwords, and read `/etc/ssh/sshd_config` while you are there. | **OK** — /etc/ssh/sshd_config is the SSH server configuration file. |
| 21 | `02-phase-networking-and-linux.md:1244` | **Why:** Service accounts exist to run processes, not to be logged into, so the expected value is `/usr/sbin/nologin`. An interactive shell on one is a finding because it is an account an attacker can actually log in as, and service accounts are frequently wea … | **OK** — /usr/sbin/nologin is the expected shell for a service account. |
...
| 24 | `03-phase-security-fundamentals.md:393` | \| `C:\Windows\System32\svchost.exe` \| The real one \| | **OK** — Windows: C:\Windows\System32 is the legitimate location for svchost.exe. |
| | | <sub>↑ \|---\|---\|<br>↓ \| `C:\Users\Public\svchost.exe` \| Not the real one \|</sub> | |
| 25 | `03-phase-security-fundamentals.md:394` | \| `C:\Users\Public\svchost.exe` \| Not the real one \| | **OK** — Windows: legitimate system binaries do not run from C:\Users\Public. |
| | | <sub>↑ \| `C:\Windows\System32\svchost.exe` \| The real one \|</sub> | |
| 26 | `03-phase-security-fundamentals.md:408` | \| **Registry Run keys** \| `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` and its `HKLM` equivalent \| Causes a program to start at login. The classic location, and still heavily used \| | **OK** — Windows: HKCU and HKLM Software\Microsoft\Windows\CurrentVersion\Run are the standard Run keys. |
| 27 | `03-phase-security-fundamentals.md:431` | \| **Processes from odd locations** \| `C:\Users\Public\`, `C:\Temp\`, `%APPDATA%`, or `/tmp` on Linux \| Legitimate system binaries do not run from user-writable directories; that is the whole point of the expectation \| | **OK** — C:\Users\Public, C:\Temp, %APPDATA% and /tmp are all user-writable locations. |
| 28 | `03-phase-security-fundamentals.md:1208` | **Why:** The phase's masquerading section makes predictability the detection: the real `svchost.exe` lives in `C:\Windows\System32`, and legitimate binaries run from their expected directories, which is exactly what makes impersonation visible. The parent-proc … | **OK** — Windows: the real svchost.exe lives in C:\Windows\System32. |
| 29 | `04-phase-hands-on-labs.md:452` | \| **The agent checks in but no events arrive** \| The log file is not in the agent's configuration, or nothing is being written to it \| On the victim, generate an event and confirm it appears in `/var/log/auth.log` \| Add the log location to `ossec.conf` on  … | **OK** — Debian/Ubuntu: /var/log/auth.log is the authentication log. |
| 30 | `04-phase-hands-on-labs.md:544` | \| 2 \| **Find it in the raw log** on the victim (`/var/log/auth.log`, or Event Viewer) \| Proving it exists *at the source* is what lets you isolate a forwarding failure from a detection failure \| | **OK** — /var/log/auth.log on Linux and Event Viewer on Windows are both valid log sources. |
| 31 | `10-phase-detection-engineering.md:41` | - Linux auditd rules and `/var/log/auth.log` interpretation | **OK** — auditd and /var/log/auth.log are Linux log sources. |
| | | <sub>↑ - Sysmon event IDs: 1, 3, 7, 8, 10, 11, 12, 13, 22<br>↓ - Syslog facilities, priorities, and severity</sub> | |
| 32 | `10-phase-detection-engineering.md:217` | \| SSH authentication \| Linux \| `/var/log/auth.log` \| High \| | **OK** — Linux: /var/log/auth.log records SSH authentication. |
...
| 33 | `10-phase-detection-engineering.md:218` | \| Cron job created \| Linux \| auditd file watch on `/etc/cron*` \| High \| | **OK** — Linux: cron job definitions live under /etc/cron*. |
...
| 34 | `10-phase-detection-engineering.md:349` | \| `/var/log/auth.log` or `/var/log/secure` \| Authentication, sudo, SSH, su \| Plain text, one event per line \| | **OK** — /var/log/auth.log and /var/log/secure record authentication, sudo, SSH and su. |
| 35 | `10-phase-detection-engineering.md:352` | \| `/var/log/syslog` or `/var/log/messages` \| General system messages \| Plain text \| | **OK** — /var/log/syslog and /var/log/messages hold general system messages. |
...
| 36 | `10-phase-detection-engineering.md:353` | \| `/etc/passwd` and `/etc/shadow` access \| Account changes \| Via auditd file watches \| | **OK** — /etc/passwd and /etc/shadow store account data. |
...
| 37 | `10-phase-detection-engineering.md:354` | \| Cron \| Scheduled job definitions and execution \| `/var/log/cron` or journald \| | **OK** — Cron execution is logged to /var/log/cron or journald. |
...
| 39 | `10-phase-detection-engineering.md:1113` | \| auditd \| Linux syscall and file auditing \| Free/open-source \| https://man7.org/linux/man-pages/man8/auditd.8.html \| Write file watches for `/etc/passwd` and `/etc/cron.d` \| journald with targeted filters \| | **OK** — Linux audit: auditd can place file watches on /etc/passwd and /etc/cron.d. |
| 40 | `11-phase-incident-response.md:456` | \| **Prefetch** \| `C:\Windows\Prefetch` \| Which programs ran, when, and how often \| | **OK** — Windows: Prefetch is at C:\Windows\Prefetch. |
...
| 42 | `11-phase-incident-response.md:462` | \| **Event logs** \| `C:\Windows\System32\winevt\Logs` \| Security, System, Application, and Sysmon records \| | **OK** — Windows: event logs are stored in C:\Windows\System32\winevt\Logs. |
| 43 | `11-phase-incident-response.md:512` | \| 09:11:40 \| LNK / Jump list \| `Invoice_4421.docm` opened from `C:\Users\jsantos\Downloads` \| | **OK** — Windows: C:\Users\jsantos\Downloads is a standard user download directory. |
...
| 44 | `11-phase-incident-response.md:513` | \| 09:11:44 \| Sysmon 11 \| `C:\Users\jsantos\AppData\Local\Temp\kx8f.tmp` created \| | **OK** — Windows: %LOCALAPPDATA%\Temp is the standard per-user temporary directory. |
...
| 45 | `11-phase-incident-response.md:952` | \| "Looked at the disk, found malware" \| "Mounted `/evidence/disk.img` read-only at 14:05 UTC. Ran `fls -o 1048576 -r -m C:/`; output to `bodyfile.txt`. Identified `C:\Users\jsantos\AppData\Local\Temp\svchost.exe` with an MFT creation timestamp of 2026-03-14  … | **OK** — Windows: C:\Users\jsantos\AppData\Local\Temp is a user-writable directory — the point the row is making. |
| 46 | `12-phase-scripting-automation.md:854` | The filters in commands 5 and 6 are doing the analytical work. A service whose binary lives outside `C:\Windows` is worth looking at; a scheduled task that has *run* recently on a machine nobody changed is worth looking at. | **OK** — Windows: legitimate service binaries run from C:\Windows or C:\Program Files. |
| 47 | `12-phase-scripting-automation.md:1133` | **What it got wrong.** Three weeks later, a real intrusion is found during an unrelated review. The entry point was a signed binary dropped into `C:\Program Files\Corp\`, with its timestamps set back by the attacker so the file appeared months old. The matchin … | **OK** — Windows: C:\Program Files is a standard installation directory. |
...
| # | Location | Text as written | Verdict |
|---|---|---|---|
| 1 | `01-phase-foundations.md:502` | \| **Technique** \| A specific way of achieving that goal \| **T1566 Phishing** under the Initial Access tactic \| | **OK** — MITRE ATT&CK: T1566 is Phishing, tactic Initial Access. |
| 2 | `01-phase-foundations.md:504` | When you see a phishing email, mapping it to T1566 joins your observation to a global, consistent vocabulary. That is why one of this phase's practice tasks is to pick five techniques and explain them. | **OK** — MITRE ATT&CK: T1566 is the Phishing technique. |
| 3 | `01-phase-foundations.md:676` | \| Initial Access \| Phishing (T1566) \| Get a foothold by convincing a user to act \| Mail filters, user reports, suspicious sender domains \| | **OK** — MITRE ATT&CK: T1566 Phishing is under Initial Access. |
| 4 | `03-phase-security-fundamentals.md:387` | That last row introduces **masquerading** — naming a malicious file after a legitimate system process to blend in. It is a MITRE ATT&CK technique (T1036). | **OK** — MITRE ATT&CK: T1036 is Masquerading. |
| 5 | `03-phase-security-fundamentals.md:875` | \| 02:14:03 \| Phishing email delivered to `j.dela+cruz@example.com` \| Mail gateway log \| Initial access attempt (ATT&CK T1566) \| | **OK** — MITRE ATT&CK: T1566 is Phishing, tactic Initial Access. |
| 6 | `03-phase-security-fundamentals.md:890` | \| ATT&CK T1566 mapping \| Phase 1 \| | **OK** — MITRE ATT&CK: T1566 is Phishing. |
| | | <sub>↑ \|---\|---\|<br>↓ \| Event IDs 4624 and 4688, logon types 3 and 10 \| Part 1 \|</sub> | |
| 7 | `04-phase-hands-on-labs.md:494` ▶ | <id>T1110</id> | **OK** — MITRE ATT&CK: T1110 is Brute Force. |
| | | <sub>↑ <mitre><br>↓ </mitre></sub> | |
| 8 | `04-phase-hands-on-labs.md:506` | - **`mitre`** — maps the detection to **T1110, Brute Force** in MITRE ATT&CK. Phase 1 introduced ATT&CK as a shared vocabulary; this is what using it looks like in practice. It means an analyst who sees this alert immediately knows what class of behaviour it r … | **OK** — MITRE ATT&CK: T1110 is Brute Force. |
| 9 | `10-phase-detection-engineering.md:152` | **T1059.001 — Command and Scripting Interpreter: PowerShell.** | **OK** — MITRE ATT&CK: T1059.001 is Command and Scripting Interpreter: PowerShell. |
| 10 | `10-phase-detection-engineering.md:508` | **ATT&CK T1543.003 — Create or Modify System Process: Windows Service.** | **OK** — MITRE ATT&CK: T1543.003 is Create or Modify System Process: Windows Service. |
| 11 | `10-phase-detection-engineering.md:553` ▶ | - https://attack.mitre.org/techniques/T1059/001/ | **OK** — attack.mitre.org/techniques/T1059/001/ — resolves to the T1059.001 page. |
| | | <sub>↑ references:<br>↓ author: Your Name</sub> | |
| 12 | `10-phase-detection-engineering.md:633` ▶ | <id>T1059.001</id> | **OK** — MITRE ATT&CK: T1059.001 is PowerShell. |
| | | <sub>↑ <mitre><br>↓ </mitre></sub> | |
| 13 | `10-phase-detection-engineering.md:735` ▶ | Invoke-AtomicTest T1543.003 -ShowDetailsBrief | **OK** — Atomic Red Team: `Invoke-AtomicTest` with `-ShowDetailsBrief` lists a technique's tests. |
| | | <sub>↑ # List the atomic tests for one technique, without running anything.</sub> | |
| 14 | `10-phase-detection-engineering.md:738` ▶ | Invoke-AtomicTest T1543.003 -TestNumbers 1 | **OK** — Atomic Red Team: `-TestNumbers` selects specific tests. |
| | | <sub>↑ # Run one test.</sub> | |
| 15 | `10-phase-detection-engineering.md:741` ▶ | Invoke-AtomicTest T1543.003 -TestNumbers 1 -Cleanup | **OK** — Atomic Red Team: `-Cleanup` reverts a test's changes. |
| | | <sub>↑ # Clean up after the test, which also tests your rule's noise profile.<br>↓ ```</sub> | |
| 16 | `10-phase-detection-engineering.md:757` ▶ | ATT&CK T1059.001 | **OK** — MITRE ATT&CK: T1059.001 is PowerShell. |
| | | <sub>↑ Title Encoded PowerShell outside known management tools</sub> | |
| 17 | `10-phase-detection-engineering.md:760` ▶ | Command Invoke-AtomicTest T1059.001 -TestNumbers 1 | **OK** — Atomic Red Team: `Invoke-AtomicTest T1059.001 -TestNumbers 1` runs the first atomic test for PowerShell. |
| | | <sub>↑ True positive test<br>↓ Result Rule fired 4 seconds after execution</sub> | |
| 18 | `10-phase-detection-engineering.md:848` | A learner writes a rule for T1110, brute force: alert when more than five failed logons occur for one account within five minutes. | **OK** — MITRE ATT&CK: T1110 is Brute Force. |
| 19 | `10-phase-detection-engineering.md:909` | \| **Technique** \| *How* they achieve the goal \| T1059 Command and Scripting Interpreter \| | **OK** — MITRE ATT&CK: T1059 is Command and Scripting Interpreter. |
...
| 20 | `10-phase-detection-engineering.md:910` | \| **Sub-technique** \| A specific variant \| T1059.001 PowerShell \| | **OK** — MITRE ATT&CK: T1059.001 is the PowerShell sub-technique. |
...
| 21 | `10-phase-detection-engineering.md:919` | \| "We detect T1059.001" \| "We detect T1059.001 via the encoded-command procedure. Non-encoded PowerShell is not covered by this rule and is covered only by process-creation logging." \| | **OK** — MITRE ATT&CK: T1059.001 is PowerShell. |
| 22 | `10-phase-detection-engineering.md:920` | \| "T1059 — covered" \| "T1059 partially covered by one sub-technique. T1059.003 (Windows Command Shell) is not covered." \| | **OK** — MITRE ATT&CK: T1059.003 is Windows Command Shell. |
| 23 | `10-phase-detection-engineering.md:931` | \| Initial Access \| T1566 Phishing \| .001 Attachment \| — \| **None** \| No email gateway feed into the SIEM \| | **OK** — MITRE ATT&CK: T1566.001 is Spearphishing Attachment. |
| 24 | `10-phase-detection-engineering.md:932` | \| Execution \| T1059 \| .001 PowerShell \| DET-003 \| Partial \| Encoded only; plain PowerShell not covered \| | **OK** — MITRE ATT&CK: T1059.001 is PowerShell. |
| 25 | `10-phase-detection-engineering.md:933` | \| Execution \| T1059 \| .003 Windows Command Shell \| — \| **None** \| Planned, low priority \| | **OK** — MITRE ATT&CK: T1059.003 is Windows Command Shell. |
...
| 26 | `10-phase-detection-engineering.md:934` | \| Persistence \| T1543 \| .003 Windows Service \| DET-001 \| **Good** \| Signature checks not yet implemented \| | **OK** — MITRE ATT&CK: T1543.003 is Windows Service. |
| 27 | `10-phase-detection-engineering.md:935` | \| Persistence \| T1053 \| .005 Scheduled Task \| DET-006 \| Partial \| Linux cron covered; Windows tasks partial \| | **OK** — MITRE ATT&CK: T1053.005 is Scheduled Task/Job: Scheduled Task. |
| 28 | `10-phase-detection-engineering.md:936` | \| Persistence \| T1136 \| .001 Local Account \| DET-002 \| **Good** \| Requires correlation with the group-change rule \| | **OK** — MITRE ATT&CK: T1136.001 is Create Account: Local Account. |
| 29 | `10-phase-detection-engineering.md:937` | \| Privilege Escalation \| T1098 \| .001 Account Manipulation \| DET-002 \| **Good** \| — \| | **OK** — MITRE ATT&CK: T1098.001 is Account Manipulation: Additional Cloud Credentials, tactics Persistence and Privilege Escalation. The cell gives the PARENT family name rather than the sub-technique's own name — the ID, the tactic and the family are all correct, and T1098 in the preceding column plus `.001` resolves to the right page. Left as written deliberately: it matches the column's use elsewhere as a technique-area label, and for a beginner 'Account Manipulation' is more legible than 'Additional Cloud Credentials'. |
...
| 30 | `10-phase-detection-engineering.md:938` | \| Credential Access \| T1003 \| .001 LSASS Memory \| DET-004 \| Partial \| Heavily filtered; a documented bypass exists \| | **OK** — MITRE ATT&CK: T1003.001 is OS Credential Dumping: LSASS Memory. |
| 31 | `10-phase-detection-engineering.md:939` | \| Discovery \| T1087 \| .001 Account Discovery \| — \| **None** \| Not detected \| | **OK** — MITRE ATT&CK: T1087.001 is Account Discovery: Local Account, tactic Discovery. As with T1098.001 above, the cell gives the parent family name; the ID and tactic are correct. |
...
| 32 | `10-phase-detection-engineering.md:940` | \| Command and Control \| T1071 \| .001 Web Protocols \| DET-008 \| Partial \| DNS-based only \| | **OK** — MITRE ATT&CK: T1071.001 is Application Layer Protocol: Web Protocols. |
| | | <sub>↑ \| Discovery \| T1087 \| .001 Account Discovery \| — \| **None** \| Not detected \|<br>↓ \| Exfiltration \| T1041 \| — \| — \| **None** \| No egress anomaly baseline \|</sub> | |
| 33 | `10-phase-detection-engineering.md:941` | \| Exfiltration \| T1041 \| — \| — \| **None** \| No egress anomaly baseline \| | **OK** — MITRE ATT&CK: T1041 is Exfiltration Over C2 Channel, tactic Exfiltration. |
| | | <sub>↑ \| Command and Control \| T1071 \| .001 Web Protocols \| DET-008 \| Partial \| DNS-based only \|</sub> | |
| 34 | `10-phase-detection-engineering.md:991` ▶ | ATT&CK T1059.001 | **OK** — MITRE ATT&CK: T1059.001 is PowerShell. |
...
| | | <sub>↑ - Networking: `ip a`, `ip route`, `ss -tulpn`, `dig`, `curl`</sub> | |
| 7 | `02-phase-networking-and-linux.md:89` | On Windows, network configuration is buried in GUI dialogs, and logging is abstracted away. On Linux, you type `ip a` and see your interfaces. You type `ss -tulpn` and see every listening service with the process that owns it. You read `/var/log/auth.log` to w … | **UNVERIFIABLE** — pedagogical comparison of Windows GUI dialogs with Linux commands |
| 8 | `02-phase-networking-and-linux.md:113` | \| `nmap -sV` \| Learning \| Unauthorised access \| | **OK** — Nmap: `-sV` enables version detection. |
| | | <sub>↑ \|---\|---\|---\|<br>↓ \| `tcpdump` on your own interface \| Learning \| Potentially unlawful interception \|</sub> | |
| 9 | `02-phase-networking-and-linux.md:114` | \| `tcpdump` on your own interface \| Learning \| Potentially unlawful interception \| | **OK** — tcpdump captures packets on a network interface. |
...
| 17 | `02-phase-networking-and-linux.md:744` | `journalctl` queries the systemd journal, which on many distributions is where logs now primarily live: | **OK** — systemd: journalctl queries the systemd journal, which is where logs primarily live on many distributions. |
| 18 | `02-phase-networking-and-linux.md:755` | A workflow worth adopting now. Open one terminal running `journalctl -u sshd -f` on your VM. Try to SSH in from your host with the wrong username, and then with the wrong password. | **OK** — systemd: `journalctl -u sshd -f` follows logs for the sshd unit. |
...
| 22 | `02-phase-networking-and-linux.md:798` | \| `sort` \| Group identical addresses together, because `uniq` only collapses *adjacent* duplicates \| | **OK** — POSIX: `sort` groups identical lines; `uniq` collapses only ADJACENT duplicates, which is why the sort is required first. |
...
| 23 | `02-phase-networking-and-linux.md:799` | \| `uniq -c` \| Replace each run with a count \| | **OK** — POSIX: `uniq -c` prefixes each run with its occurrence count. |
...
| 24 | `02-phase-networking-and-linux.md:800` | \| `sort -rn` \| Sort numerically, highest first \| | **OK** — POSIX: `sort -rn` sorts numerically (`-n`) and reverses (`-r`), giving highest first. |
...
| 28 | `02-phase-networking-and-linux.md:871` | `ss -tulpn` is the Linux equivalent of a port scan against your own machine. Phase task 4 asks you to run it and identify every listening service. Run it with `sudo`: the `p` flag needs root to read the owning process for sockets that belong to other users, an … | **OK** — iproute2: `ss -tulpn` lists listening sockets; `-p` shows the owning process and needs root to see sockets owned by other users. |
...
| | | <sub>↑ \| `grep -oE 'from [0-9.]+'` \| Keep only the `from <address>` fragment \|<br>↓ \| `sort` \| Order them so identical addresses are adjacent \|</sub> | |
| 32 | `02-phase-networking-and-linux.md:930` | \| `sort` \| Order them so identical addresses are adjacent \| | **OK** — POSIX: `sort` orders lines so identical addresses become adjacent. |
| | | <sub>↑ \| `awk '{print $2}'` \| Extract the address — field 2 of `from 203.0.113.45` \|<br>↓ \| `uniq -c` \| Collapse duplicates and count each \|</sub> | |
| 33 | `02-phase-networking-and-linux.md:931` | \| `uniq -c` \| Collapse duplicates and count each \| | **OK** — POSIX: `uniq -c` collapses duplicates and counts them. |
| | | <sub>↑ \| `sort` \| Order them so identical addresses are adjacent \|<br>↓ \| `sort -rn` \| Sort by count, descending \|</sub> | |
| 34 | `02-phase-networking-and-linux.md:932` | \| `sort -rn` \| Sort by count, descending \| | **OK** — POSIX: `sort -rn` sorts by count, descending. |
...
| 39 | `02-phase-networking-and-linux.md:1139` | - **`ss -tulpn` is a self-scan, and `0.0.0.0` versus `127.0.0.1` is the finding.** A service bound to all interfaces is reachable from the network; one bound to localhost is not. | **OK** — iproute2: `ss -tulpn` shows listening sockets with bind addresses. `0.0.0.0` means all interfaces; `127.0.0.1` means localhost only. |
...
| 41 | `02-phase-networking-and-linux.md:1154` | 4. **Run `ss -tulpn` and identify every listener** (task 4). For each one, decide whether it *should* be listening and whether it is bound to all interfaces. This is host hardening in miniature. | **OK** — iproute2: `ss -tulpn` lists every listening socket. |
| 42 | `02-phase-networking-and-linux.md:1166` | 13. **Prove key-only SSH** (task 13). Generate a keypair, install the public key in `authorized_keys`, disable password authentication in `sshd_config`, and demonstrate that a password login now fails. "Demonstrate the failure" is the checklist item — anyone c … | **OK** — OpenSSH: key-based auth uses authorized_keys; `PasswordAuthentication no` in sshd_config disables password login. |
| 43 | `02-phase-networking-and-linux.md:1167` | 14. **Capture and filter with `tcpdump`** (task 14). Save to a `.pcap`, open it in Wireshark, and filter to a single protocol. This is the command-line half of the packet work, and it is what you would actually have on a server with no GUI. | **OK** — tcpdump writes .pcap files, which Wireshark opens. |
| 44 | `02-phase-networking-and-linux.md:1198` | 7. Run `nmap -sV` against your own VM IP only. <!-- id: cyber-02-t07 band: quick energy: normal --> | **OK** — Nmap: `-sV` enables version detection. |
...
| 46 | `02-phase-networking-and-linux.md:1205` | 14. Use `tcpdump` to capture traffic on your VM's interface, save it to a `.pcap`, then open it in Wireshark and filter to a single protocol. <!-- id: cyber-02-t14 band: focused energy: normal --> | **OK** — tcpdump writes .pcap files; Wireshark opens and filters them. |
...
| 48 | `03-phase-security-fundamentals.md:302` | In Linux the equivalent evidence lives where Phase 2 showed you: `/var/log/auth.log` or `/var/log/secure`, read through `journalctl -u sshd`. | **OK** — /var/log/auth.log (Debian/Ubuntu) and /var/log/secure (RHEL/CentOS) are the standard authentication logs. |
...
| 51 | `03-phase-security-fundamentals.md:1037` | Take five findings — from the `nmap -sV` scan you ran against your own VM in Phase 2, from a Nessus/OpenVAS scan of your own lab, or from the CVE exercise in Phase 1 — and score them. | **OK** — Nmap: `-sV` performs version detection. |
| 52 | `04-phase-hands-on-labs.md:238` | \| Linux VM \| `ss -tulpn` \| | **OK** — iproute2: `ss -tulpn` shows listening sockets. |
...
| | | <sub>↑ - Raw excerpt from /var/log/auth.log, three of the six lines, captured with<br>↓ - Wazuh alert for rule 100001, level 10, timestamped 14:32:11 UTC</sub> | |
| 60 | `04-phase-hands-on-labs.md:831` ▶ | **Observed:** `ossec.log` shows `Unable to connect to manager`. `ss -tulpn` | **OK** — iproute2: `ss -tulpn` shows listening sockets; nothing on 1514 means the agent cannot connect. Consistent with the corpus's own Wazuh troubleshooting scenario. |
...
| 61 | `06-phase-portfolio-projects.md:1020` | \| 14:09 \| Generated test traffic \| `nmap -sS 192.168.56.10` \| 3 open ports found \| `02-scan.png` \| | **OK** — Nmap: `-sS` performs a TCP SYN scan. |
...
| 62 | `09-phase-cloud-and-identity.md:235` | **Before any of this works you need the AWS CLI**: install it, then run `aws configure` once with the IAM user's access key, secret, and a default region. Every command below reads those stored credentials, so a bare `aws` command with no configuration fails i … | **OK** — AWS: `aws configure` stores the access key, secret and default region for subsequent commands. |
| 63 | `09-phase-cloud-and-identity.md:268` | Two details in that third command are worth naming, because both are the kind of thing that makes an audit script quietly wrong. The version id has to be fetched rather than assumed, and the `grep` pattern is written loosely enough to catch the several shapes  … | **UNVERIFIABLE** — curriculum narrative about one command's context |
| 64 | `09-phase-cloud-and-identity.md:536` | \| `userAgent` \| What client made the call \| `aws-cli` at 3 a.m. from a service account is worth a look \| | **OK** — AWS CloudTrail records `userAgent` on each event. |
...
| 65 | `09-phase-cloud-and-identity.md:581` | \| Was it a human or automation? \| `userAgent` of `aws-cli` plus the pattern suggests scripted activity, possibly a human at a terminal \| | **OK** — AWS CloudTrail records `userAgent`, which distinguishes scripted from interactive callers. |
| 66 | `10-phase-detection-engineering.md:341` | One practical note on the filename: the well-known community configurations publish their file under names such as `sysmonconfig.xml` and `sysmonconfig-export.xml`, and the name is only ever a convention — nothing reads it. What matters is that the file you pa … | **OK** — Sysinternals: a Sysmon configuration file name is a convention; nothing reads it. |
| 67 | `10-phase-detection-engineering.md:350` | \| `auditd` with `auditctl` rules \| Syscalls and file watches, configurable in detail \| `ausearch` and `aureport` \| | **OK** — Linux audit: `auditd` with `auditctl` rules configures syscall and file-watch auditing; `ausearch` and `aureport` query the logs. |
| 68 | `10-phase-detection-engineering.md:351` | \| `journald` \| Systemd service logs, kernel messages \| `journalctl` with filters \| | **OK** — systemd: `journald` collects service and kernel logs; `journalctl` queries them with filters. |
...
| 69 | `10-phase-detection-engineering.md:356` | **This half needs a Linux host.** The Windows telemetry above runs on your own PC; auditd does not. If you do not have a Linux machine yet, do this section after Phase 11, which walks you through building one in VirtualBox — or on WSL2, which is enough for `au … | **OK** — Linux audit: auditd is a Linux-only daemon and does not run on Windows. |
| 70 | `10-phase-detection-engineering.md:358` | The auditd rules worth writing first, because they cover the persistence and privilege-escalation behaviours that matter. Put them in `/etc/audit/rules.d/10-lab.rules`, load them with `augenrules --load`, and confirm they are live with `auditctl -l` — a rule t … | **OK** — Linux audit: rules live in /etc/audit/rules.d/, are loaded with `augenrules --load`, and listed with `auditctl -l`. |
...
| | | <sub>↑ - Secrets management: environment variables, `.env` files, and `.gitignore`<br>↓ - PowerShell objects and pipes, and why `Select-Object` beats text parsing</sub> | |
| 75 | `12-phase-scripting-automation.md:47` | - PowerShell objects and pipes, and why `Select-Object` beats text parsing | **OK** — Microsoft: `Select-Object` operates on objects from the pipeline, which is why it beats parsing formatted text. |
...
| 76 | `12-phase-scripting-automation.md:48` | - Calling REST APIs from PowerShell with `Invoke-RestMethod` | **OK** — Microsoft: `Invoke-RestMethod` sends HTTP and REST requests. |
...
| | | <sub>↑ - Calling REST APIs from PowerShell with `Invoke-RestMethod`<br>↓ - Defanging indicators so they are safe to paste into reports</sub> | |
| 78 | `12-phase-scripting-automation.md:739` | That `Export-Csv` line is a complete deliverable. Compare it with the text-parsing approach, which requires fixed-width column offsets that change between Windows versions. | **OK** — Microsoft: `Export-Csv` serialises objects to CSV. |
| 79 | `12-phase-scripting-automation.md:743` | `Get-WinEvent` is the workhorse, and its filtering syntax is worth learning properly because the difference between a good and a bad filter is minutes versus hours. | **OK** — Microsoft: `Get-WinEvent -FilterHashtable` filters at the source and is markedly faster than piping to Where-Object. |
...
| 83 | `12-phase-scripting-automation.md:1434` | \| CyberChef \| Decoding and transformation \| Free/open-source \| https://gchq.github.io/CyberChef/ \| Decode a base64 command line by hand before scripting it \| Python `base64` and `codecs` \| | **OK** — GCHQ: CyberChef is a free, open-source web app for decoding and transforming data. |
| 84 | `14-phase-web-app-security.md:52` | - SQL injection testing with manual payloads and `sqlmap` on authorised targets | **OK** — sqlmap is an open-source SQL injection detection and exploitation tool. |
...
| 86 | `14-phase-web-app-security.md:841` | **On Windows, Docker needs WSL2.** Docker Desktop offers to install it on first run, which needs administrator rights and a reboot; if you cannot get either, run Juice Shop inside your Phase 2 Linux VM instead, where the same `docker run` line works. Git Bash  … | **OK** — Docker: Docker Desktop on Windows requires WSL2. |
| 87 | `14-phase-web-app-security.md:865` | **Two cautions about `sqlmap` that matter more than the command.** It sends a large volume of requests, so it will be detected and rate-limited. On a fragile application it can also disrupt service. And running it against anything other than a target you own o … | **OK** — sqlmap sends a high volume of requests, so it is easily detected and rate-limited. |
| 88 | `14-phase-web-app-security.md:1092` | \| `script-src` \| Where JavaScript may be loaded from \| | **OK** — W3C CSP: `script-src` specifies the valid sources for JavaScript. |
...
| 89 | `14-phase-web-app-security.md:1133` | \| Requests with unusual user agents or methods \| `sqlmap`, `nikto`, or unexpected `PUT` \| Automated scanning \| | **OK** — sqlmap and nikto set identifiable User-Agent strings, which is why they appear in this detection signal. |
| 90 | `14-phase-web-app-security.md:1304` | 5. **Work through injection properly** (tasks 5 and 6): the SQL injection Apprentice labs by hand first, then confirm one with `sqlmap` against Juice Shop on localhost and compare what the tool reports with what you found manually. | **OK** — sqlmap automates SQL injection detection and exploitation. |
| 91 | `14-phase-web-app-security.md:1324` | \| curl \| Command-line HTTP client \| Free/open-source \| https://curl.se/ \| Fetch a response with headers and inspect the security headers \| PowerShell `Invoke-WebRequest` \| | **OK** — curl is a free, open-source command-line HTTP client. |
| 92 | `15-phase-ot-ics-security.md:40` | - Capture and analyse control traffic passively with Wireshark, `tcpdump`, and Zeek without sending a single packet at a device. | **OK** — Wireshark, tcpdump and Zeek are all passive network monitoring tools — none transmits to the monitored device. |
| 93 | `15-phase-ot-ics-security.md:114` | \| Wireshark, `tcpdump`, and Zeek on OT captures \| 14–18 \| The skill that gets used on day one \| | **UNVERIFIABLE** — “the skill that gets used on day one” is a judgement about career relevance |
...
| 94 | `15-phase-ot-ics-security.md:823` | \| Capture \| `tcpdump`, or a managed switch's mirror port \| Free \| The raw packets \| | **OK** — tcpdump captures raw packets; a switch mirror port is the passive alternative. |
| | | <sub>↑ \|---\|---\|---\|---\|<br>↓ \| Deep inspection \| Wireshark \| Free \| Understanding one exchange completely \|</sub> | |
| 95 | `15-phase-ot-ics-security.md:1121` | \| Wireshark \| Packet capture and protocol dissection \| Free/open-source \| https://www.wireshark.org/ \| Dissect a Modbus read and a write and annotate the function codes \| `tshark` on the command line \| | **OK** — Wireshark is a free, open-source packet analyser with protocol dissection. |
| 96 | `15-phase-ot-ics-security.md:1122` | \| Zeek \| Network security monitoring that produces structured logs \| Free/open-source \| https://zeek.org/ \| Capture a lab session and find every connection in `conn.log` \| Suricata in its logging mode, or `tcpdump` plus scripts \| | **OK** — Zeek is free and open source, and produces structured logs including conn.log. |
| 97 | `15-phase-ot-ics-security.md:1123` | \| Suricata \| Signature and protocol-aware intrusion detection \| Free/open-source \| https://suricata.io/ \| Write a rule that fires on Modbus write function codes \| Zeek scripts, or Python over `tshark` output \| | **OK** — Suricata is a free, open-source signature and protocol-aware IDS/IPS. |
| 98 | `15-phase-ot-ics-security.md:1124` | \| `tcpdump` \| Command-line packet capture \| Free/open-source \| https://www.tcpdump.org/ \| Capture Modbus traffic to a file for later analysis \| Wireshark's own capture, or `dumpcap` \| | **OK** — tcpdump is free, open source, and command-line packet capture. |
| 99 | `15-phase-ot-ics-security.md:1318` | Every tool this phase needs is free. Wireshark, Zeek, Suricata, `tcpdump`, and GRASSMARLIN are open source, and together they are a genuine passive monitoring stack rather than a demo. OpenPLC and the free protocol simulators give you a controller to talk to w … | **OK** — Wireshark, Zeek, Suricata, tcpdump and GRASSMARLIN are all free and open source. |
