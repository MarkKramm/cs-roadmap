...
**Status: verification in progress — 282 of 761 extracted claims settled, 125 outstanding.**

Six of the nine claim classes are complete and are **no longer emitted as worklists**:
standards/frameworks (105 rows), CVE identifiers (4), cryptography (5), product versions (9),
command/cmdlet usage (60) and security tool commands and flags (99) — **282 rows cleared, zero
`WRONG` verdicts.** Three classes remain — ATT&CK identifiers, protocol behaviour, registry/paths
— and now fit in a **single** message at
[`claims-to-verify-cyber/bundles/01-bundle.md`](claims-to-verify-cyber/bundles/01-bundle.md).
...
**Zero `WRONG` verdicts in 282 rows, and the 21% `UNVERIFIABLE` rate is the honest part.** Almost
...
them, and lowest in the two command classes (7% and 6%), where a line either names a real flag or
does not. The pack's own rules say not to invent a verdict to fill a column.
...
> derives every published figure from the rows and CI fails if the table disagrees.
...
product-version claim, a command-usage claim and a tool-flag claim at once — so **232 distinct
locations fill 282 rows.** No row carries a verdict that was not explicitly supplied, and
